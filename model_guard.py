...
PERSISTENT, MUTABLE Modal Volume used as the Hugging Face cache
(`llmlingua2-hf-cache`, `attentionrag-hf-cache`, `turboquant-hf-cache`). The
volume is populate-once-then-read and is committed back from inside the GPU
container, so whatever sits in it at load time is what gets loaded. Before this
module none of the load sites pinned a revision, forced safetensors, or checked
a digest - the cache was an unverified trust boundary.
...
Modal images pip-install `transformers` unpinned. So we take the file out of
the decision instead of trusting the loader.
...
# --------------------------------------------------------------------------- #
# 6. Convenience wrapper for the Modal @enter loaders                          #
# --------------------------------------------------------------------------- #
def llmlingua_model_config(model_id: str, **extra) -> dict:
    """`model_config` for llmlingua's `PromptCompressor`, with remote code OFF.

    llmlingua 0.2.2 defaults this to ON. From its own source
    (`llmlingua/prompt_compressor.py`, load_model):
