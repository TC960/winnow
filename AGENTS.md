...
- `web/components/learn/ChatPanel.tsx`: Codex chat grounded in the compressed transcript.
...
2. Deterministic and cached: same input plus config gives same output. Cache per-turn embeddings and summaries by content hash. Identical Codex sampling params across variants; vary only a seed.
